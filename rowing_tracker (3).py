...
# File where workouts are saved between runs
DATA_FILE = "workouts.json"

# Valid workout types. This prevents inconsistent labels like "steady state" vs. "steady_state".
VALID_TYPES = ["steady_state", "interval", "test_piece", "cross_training"]


def format_split(seconds):
    """Convert seconds into rowing split format, like 132 seconds -> 2:12."""
...
def parse_split(split_text):
    """Convert user input like 2:12 into total seconds."""
    try:
        minutes, seconds = split_text.split(":")
        minutes = int(minutes)
        seconds = int(seconds)

        # Seconds must be between 0 and 59 for normal time format.
...
def valid_date(date_text):
    """Check whether the user entered the date in YYYY-MM-DD format."""
    try:
...
def save_workouts(workouts):
    """Save workouts to a JSON file so data is not lost when the program ends."""
    with open(DATA_FILE, "w") as f:
        json.dump(workouts, f, indent=4)


def load_workouts():
    """Load workouts from the JSON file. If no file exists yet, start empty."""
    try:
...
def add_workout(workouts, date, workout_type, distance_m, avg_split_sec, stroke_rate, notes):
    """Validate and add a workout dictionary to the workout list."""
...
def add_workout_from_user(workouts):
    """Ask the user for workout information and add it if valid."""
    print("\nAdd a new workout")
    print("-" * 30)
...
def display_workouts(workouts):
    """Display all workouts in date order."""
    if not workouts:
...
def filter_by_type(workouts, workout_type):
    """Return only workouts that match a certain workout type."""
    return [w for w in workouts if w["type"] == workout_type]


def average_split(workouts, workout_type=None):
    """Calculate average split overall or for one workout type."""
    filtered = workouts
...
def average_stroke_rate(workouts, workout_type=None):
    """Calculate average stroke rate overall or for one workout type."""
    filtered = workouts
...
def total_distance(workouts, workout_type=None):
    """Calculate total meters rowed overall or for one workout type."""
    filtered = workouts
...
def best_split(workouts, workout_type=None):
    """Find the workout with the fastest average split."""
    filtered = workouts
...
def summary_by_type(workouts):
    """Show useful statistics for each workout type."""
    if not workouts:
...
def consistency_score(workouts, workout_type=None):
    """Measure how much a rower's splits vary from their average split."""
    filtered = workouts
    ...
    # Lower average difference means more consistent splits.
...
def predict_2k(workouts):
    ...
    This is intentionally simple and should be treated as an estimate,
    not a guaranteed race result.
    """
    useful_workouts = [
        w for w in workouts
    ...
    # Add 2 seconds to make the estimate conservative.
...
def choose_workout_type():
    """Let the user choose one workout type or all workouts."""
    print("\nChoose workout type:")
    print("all")
...
def add_sample_data(workouts):
    """Add sample data for demonstrations and testing."""
    sample_workouts = [
